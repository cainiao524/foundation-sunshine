...
namespace proc {
  ...
  proc_t::apply_app_display_profile(int app_id, rtsp_stream::launch_session_t &launch_session) const {
    const auto profiles = _display_profiles->load();
    if (!profiles) return false;
    const auto it = profiles->find(std::to_string(app_id));
    if (it == profiles->end()) return false;
    launch_session.app_display_profile = it->second;
    if (!it->second) return true;
    const auto &profile = *it->second;
    launch_session.use_vdd = profile.target == app_display::target_e::virtual_display;
    ...
    if (!launch_session.use_vdd && !profile.output_name.empty()) {
      launch_session.env["SUNSHINE_CLIENT_DISPLAY_NAME"] = profile.output_name;
    }
    if (profile.topology) launch_session.custom_screen_mode = static_cast<int>(*profile.topology);
    // Host display overrides do not rewrite the negotiated stream viewport.
    BOOST_LOG(info) << "Attached app display profile for app " << app_id;
    return true;
  }
  ...
  proc_t::set_apps(std::vector<ctx_t> apps) {
    ...
    auto profiles = std::make_shared<display_profiles_t>();
    for (const auto &app : apps) profiles->emplace(app.id, app.display_profile);
    _display_profiles->store(std::move(profiles));
    _apps = std::move(apps);
  }
  ...
  parse(const std::string &file_name) {
    ...
    try {
      ...
      for (auto &[_, app_node] : apps_node) {
        ...
        auto display_profile = app_display::parse(app_node);
        ...
        ctx.display_profile = std::move(display_profile);
